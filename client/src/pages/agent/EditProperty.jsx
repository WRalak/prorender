...
import { ArrowLeftIcon, PhotoIcon, XMarkIcon, MapPinIcon, CurrencyDollarIcon, HomeIcon, BuildingOfficeIcon, CalendarIcon, CheckCircleIcon, ExclamationTriangleIcon } from "@heroicons/react/24/outline";
...
const EditProperty = () => {
    ...
    () => propertyAPI.getPropertyById(id),
    {
      ...
      onSuccess: (data) => {
        ...
        reset({
          ...
          pricing: {
            rent: data.pricing?.rent || 0,
            deposit: data.pricing?.deposit || 0,
            applicationFee: data.pricing?.applicationFee || 0,
            lateFee: data.pricing?.lateFee || 0,
          },
          details: {
            yearBuilt: data.details?.yearBuilt || '',
            parking: data.details?.parking || 'none',
            petPolicy: data.details?.petPolicy || 'allowed',
            laundry: data.details?.laundry || 'none',
            hvac: data.details?.hvac || 'central',
            furnished: data.details?.furnished || false,
            additionalDetails: data.details?.additionalDetails || '',
          },
        });
        
        // Set images and amenities
  ...
  const updatePropertyMutation = useMutation(
    (propertyData) => propertyAPI.updateProperty(id, propertyData),
    {
      onSuccess: (data) => {
  ...
  const steps = [
    ...
    { id: 5, name: 'Amenities', icon: CheckCircleIcon },
  ];
  ...
  if (isLoading) {
    return <LoadingSpinner size="large" text="Loading property..." />;
  }
