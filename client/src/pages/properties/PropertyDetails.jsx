...
import { MapPinIcon, HomeIcon, BuildingOfficeIcon, CalendarIcon, CurrencyDollarIcon, HeartIcon, ShareIcon, CameraIcon, CheckCircleIcon, ClockIcon, StarIcon } from "@heroicons/react/24/outline";
...
const PropertyDetails = () => {
  ...
  const contactOwnerMutation = useMutation(
    ({ propertyId, message }) => propertyAPI.contactOwner(propertyId, message),
    {
      onSuccess: () => {
  ...
  const handleContactOwner = (message) => {
    if (!user) {
      toast.error('Please login to contact property owner');
      return;
    }
    contactOwnerMutation.mutate({ propertyId: id, message });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {showContactModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Contact Property Owner</h3>
            <form
              onSubmit={(e) => {
                e.preventDefault();
                const formData = new FormData(e.target);
                handleContactOwner(formData.get('message'));
              }}
              className="space-y-4"
            >
              <div>
              ...
              <div className="flex space-x-3">
                ...
                <Button
                  type="submit"
                  loading={contactOwnerMutation.isLoading}
                >
                  Send Message
                </Button>
              </div>
      ...
      {showApplicationModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Apply for Property</h3>
            <div className="space-y-4">
              ...
              <p className="text-sm text-gray-600">
                Clicking "Submit Application" will take you to the application form where you can
                provide your details and complete the application process.
              </p>
              <div className="flex space-x-3">
                <Button
                  variant="outline"
